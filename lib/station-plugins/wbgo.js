...
// WBGO Station Adapter
// ====================
//
//  Some quick notes on query params:
//
// - t is ignored. likely used as a cachebuster.
// - before: ISO date in WBGO time zone. (ex: 2017-10-14+12:00:00), pins
// - after: ISO date in WBGO time zone. requires use of limit=-1, which is
//   kinda "dangerous". cannot be used in combination with before.
// - before + after --> uses before, ignores after, and applies limit if
...
function getData() {
  ...
  var buildRequestURL = function() {
    var before_param = (this.rangeQueue) ?
      'before=' + this.rangeQueue.shift().format('YYYY-MM-DD+HH:mm:ss') :
      "";
